"""测试模块，用于非校园网环境下使用NetworkManager类进行网络连接测试。"""
import time
...
class NetworkManager:
    ...
    def check_network(self):
        ...
        """
        url = self.get_auth_urls()['test_url']
        headers = self.get_data()['test_headers']

        try:
            # 发送 GET 请求检测网络状态
            response = requests.get(url=url, headers=headers, timeout=self.RETRY_INTERVAL)
            # 判断状态码是否为 200 且响应 URL 不包含认证域名
            if not (response.status_code == 200 and self.AUTH_DOMAIN not in response.url):
    ...
    def get_auth_urls(self):
        ...
        try:
            ...
            return {
                'login': f'https://{self.AUTH_DOMAIN}/webauth.do?wlanacip=172.16.1.82&wlanuserip=',
                'disconnect': f'https://{self.AUTH_DOMAIN}/webdisconn.do?wlanacip=172.16.1.82&wlanuserip=',
